...
The application can benefit from hardware-accelerated video transcoding using Vulkan Video native acceleration on Linux machines with a Vulkan-capable GPU (NVIDIA or AMD) with Mesa drivers and Vulkan Video extension support.
...
1. **Build the multimedia processing pipeline** — using existing Membrane components, we will build a pipeline
   that ingests an RTMP stream, transcodes it into multiple resolutions and bitrates,
...
> **Note**: Starting from `membrane_transcoder_plugin` v0.4.0, a single Transcoder instance can produce multiple output variants, which simplifies the pipeline structure significantly.
...
- `Bandit` HTTP server — a minimal [Plug.Router](https://hexdocs.pm/plug/Plug.Router.html)-based server
  ([`http_server.ex`](https://github.com/membraneframework-labs/ex_broadcaster/blob/main/lib/ex_broadcaster/http_server.ex))
...
that converge into a single `Membrane.Transcoder` with multiple outputs, then into per-variant CMAF muxers before reaching the shared HLS sink.
...
graph LR
    ...
    subgraph "1080p variant"
        CMAF1080["Membrane.MP4.Muxer.CMAF\n(1080p)"]
    end

    subgraph "720p variant"
        CMAF720["Membrane.MP4.Muxer.CMAF\n(720p)"]
    end

    subgraph "480p variant"
        CMAF480["Membrane.MP4.Muxer.CMAF\n(480p)"]
    end
    ...
    Transcoder -- "output(:p720)"  --> CMAF720
    Transcoder -- "output(:p480)"  --> CMAF480

    AudioTee -- "output(:p1080)" --> CMAF1080
    AudioTee -- "output(:p720)"  --> CMAF720
    AudioTee -- "output(:p480)"  --> CMAF480
...
# lib/ex_broadcaster/pipeline.ex
  defp build_spec(client_ref, storage, segment_duration) do
    ...
    hls_sink =
      child(:hls_sink, %HTTPAdaptiveStreamSink{
        manifest_config: %HTTPAdaptiveStreamSink.ManifestConfig{
          name: "index",
          module: HTTPAdaptiveStream.HLS
        },
        track_config: %HTTPAdaptiveStreamSink.TrackConfig{},
        storage: storage
      })
...
# lib/ex_broadcaster/pipeline.ex
  defp build_variant_spec(variant, segment_duration) do
    ...
    # Audio path: from audio tee to CMAF muxer
    audio_to_muxer =
      get_child(:audio_tee)
      |> via_out(Pad.ref(:output, id))
